...
public:
	__declspec(dllimport) static void* SetWindowLong(void* hWnd, int nIndex, int dwNewLong);

	//VB TO C++ CONVERTER NOTE: The following .NET DllImport attribute was converted using the Microsoft-specific __declspec(dllimport):
	//[DllImport("user32.dll")]
	//VB TO C++ CONVERTER NOTE: The following .NET attribute has no direct equivalent in native C++:
	//ORIGINAL LINE: <DllImport("user32.dll")> Public Shared Function GetWindowRect(ByVal hWnd As IntPtr, <Out> ByRef lpRect As RECT) As <MarshalAs(UnmanagedType.Bool)> Boolean
	__declspec(dllimport) static bool GetWindowRect(void* hWnd, RECT &lpRect);

	RECT windowsRect;
	static constexpr int PyGame = 0x20DCA48;
private:
	...
	void Form1_Load(void *sender, EventArgs *e)
	{
...
public:
	...
	static bool WorldToScreen(std::vector<float> &_viewMatrix, Vect3 *pos, Vect2 *&screen, int windowWidth, int windowHeight)
	{
		screen = new Vect2();
